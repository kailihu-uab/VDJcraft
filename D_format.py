...
#  with open(output_filename, 'a+') as g:
        ...
        for line in f:
            ...
            if match:
                if qstart != 0:
                    out1=(query+'\t'+sname+'\t'+le+'\t'+score+'\t'+expect+'\t'+iden+'\t'+idenper+'\t'+gap+'\t'+gapper+'\t'+strand+'\t'+qstart+'\t'+qend+'\t'+qseq+'\t'+sstart+'\t'+send+'\t'+sseq)
                    g.write(out1+'\n')
                    sname = qseq = sseq = ""
                    le = 0
                    score = expect = 0
            ...
            if match:
                if qstart != 0:
                    out2=(query+'\t'+sname+'\t'+le+'\t'+score+'\t'+expect+'\t'+iden+'\t'+idenper+'\t'+gap+'\t'+gapper+'\t'+strand+'\t'+qstart+"\t"+qend+"\t"+qseq+"\t"+sstart+"\t"+send+"\t"+sseq)
                    g.write(out2+'\n')
                    sname = qseq = sseq = ""
                    le = 0
                    score = expect = 0
