...
def process_line(line,cutoff):
    ...
    if identpct > cutoff:
        ...
        return Query, gene_name,score, group_key,length,strand,querystart,queryend,seq,identpct,line
    else:
        return None


def update_line(prev_line, score, identpct):
    if prev_line is None:
        return True

    prev_score = prev_line[2]
    prev_ident = prev_line[9]

    if score > prev_score:
        return True

    if 500 < score <= prev_score and identpct > prev_ident:
        return True
...
def top_extract(input_file,add_file,output_file, output_vj, out_vgene, cutoff):
    ...
    with open(input_file, 'r') as file, open(add_file, 'w') as add, open(output_file, 'w') as out, open(output_vj, 'w') as outvj, open(out_vgene, 'w') as outv:
        ...
        for line in file:
            ...
            if result:
                Query, gene_name,score,group_key,length,strand,querystart,queryend,seq,identpct,line = result
                if current_query is None or Query != current_query:
                    # Output the data for the previous query
                    if current_query is not None:
                        if v_max_line is not None and v_max_line[2] <= 300:
                            pass  # skip this query
                        else:
      ...
      #          if group_key == 'V' and (v_max_line is None or score > v_max_line[2] or identpct > v_max_line[9]):
                if group_key == 'V' and update_line(v_max_line, score, identpct):
                    v_max_line = (Query, gene_name, score, group_key, length, strand, querystart, queryend, seq, identpct, line)
