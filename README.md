...
VDJcraft is a tool for VDJC identification with long-read transcriptome sequencing data. The input should be sequencing reads file: fastq or fa (PacBio Iso-Seq, Nanopore, or mixed platform). The output is a list of confident VDJC gene list and their CDR nt sequences and amino acid sequences. By default, VDJcraft uses Human GRCh38 and IMGT VDJC database.
...
optional arguments:
    ...
    -I, --fastq           Input fastq.
    -b, --bam             Input bam alignment file
    -ref, --ref           Enter your human reference genome.
    -o, --outs            Set output folder.
    -f, --outfile         Output result file.
    -ont, --ontfq         Enter your input ONT file.
    -t, --t2t             If using T2T as human refence genome, has to use this which corresponds to T2T BED file.
    -hs37, --hs37         If using GRCh37 as human refence genome, has to use this which corresponds to hg37 BED file.
    -M, --Mm              If using GRCm39 as mouse refence genome, has to use this which corresponds to GRCm39 BED file.
...
```
./VDJcraft -I test.fastq  -ref human_refence.fa(eg.GRCh38.p14.genome.fa) -o output_folder -f output
```
Or with custom reference genome and filteration cutoff for VJC and D genes:
```
./VDJcraft -I test.fastq -t -p 95 -d 85 -ref T2T.fa -o output_folder -f output
```

VDJcraft can be applied with ONT fastq or fasta file with parameter '-ont':
```
./VDJcraft -ont ONT.fastq  -ref human_refence.fa -o output_folder -f output
```
...
#### 3. --R, IMGT database
Input IMGT database can be generated by users for second path of local realignment. Users are welcome to generate IMGT database fasta file to be blastn database. Make sure to index the custom self_generate_IMGT.fa by 'makeblastdb -in self_generate_IMGT.fa -dbtype nucl -out self_generate_IMGT'.
By default, VDJcraft using latest version updated in 2025 on IMGT is provided.
```
./VDJcraft -I test.fastq -R self_generate_IMGT.fa -ref GRCh38.p14.genome.fa -o output_folder -f output
```
